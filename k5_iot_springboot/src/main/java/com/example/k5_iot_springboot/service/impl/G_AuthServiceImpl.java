...
public class G_AuthServiceImpl implements G_AuthService {
    ...
    // private final WebSecurityConfig BCryptPasswordEncoder;
    ...
    public void signUp(SignUpRequest req) {
        ...
        if (userRepository.existsByEmail(req.email())) {
            throw new IllegalArgumentException("이미사용중인 이메일 입니다.");
        }
    ...
    public ResponseDto<SignInResponse> signIn(SignInRequest req) {
        ...
        // >>>>>>> JWT 발급
        Authentication auth = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(req.loginId(), req.password()));
        // 2) 권한 문자열 추출
        Set<String> roles = auth.getAuthorities().stream()
                .map(GrantedAuthority::getAuthority)
                .collect(Collectors.toSet());

        // 3) JWT 발급(username=loginId 포함)
        ...
        return ResponseDto.setSuccess("로그인성공", response);
    }
}

    ...
    public ResponseEntity<?> logout(
        ...
        if(userPrincipal == null){
            ...
            response.getWriter().write("""
                    {"success": false, "message": "로그인정보가 없습니다"}
                    """);
        ...
        // - 첫 버째 인자(키), 두번째 인자(값)을 통해 정보를 전달
        // - 자바 서블릿 환경에서 쿠키를 다루익 위한 클래스
        var cookie = new jakarta.servlet.http.Cookie("refreshToke", null);

        cookie.setHttpOnly(true); // 쿠키 설정 보안 강화 - 쿠키의 JS 접근 여부 설정(true:접근불가)
        // cookie.setSecure(true);  // 쿠키 설정 보안 강화 - HTTPS 통신 환경에서만 서버로 전송
        cookie.setPath("/"); // 쿠키의 적용 범위를 특정 경로로 제한
        cookie.setMaxAge(0); // 쿠키 유효시간 절정 - 0: 즉시 삭제
        ...
        try {
            ...
            if (refreshToken == null) {
                return ResponseEntity.status(401).body(Map.of(
                        "success", false, "message", "Refresh Token이 존재하지 않습니다."
                ));
            }
            ...
            return ResponseEntity.ok(Map.of(
                    "success", true, "data", Map.of("accessToken", newAccessToken)
            ));
        } catch (Exception e) {
            // 검증 실패 -> 401 응답 반환 Unauthorized
            return ResponseEntity.status(401).body(Map.of(

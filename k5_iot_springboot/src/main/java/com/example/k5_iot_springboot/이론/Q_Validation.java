...
/*
    ...
    +) @RestControllerAdvice 또는 GlobalExceptionHandler로 처리가능

    4. 주요 Validation 어노테이션

    +) 각 어노테이션은 message 속성을 가짐
        - 검증 실패시 사용자에게 보여줄 오류 메시지를 직접 지정한느 역할
    ...
    cf) NonNull
        : Lombok 어노테이션
        - null 값이 아니어야 함
        - 반드시 초기화가 이루어져야 하기 때문에 @RequiredArgsConstructor 와 주로 사용

    4) @Size
        : 문자열, 배열, List, Set 등의 길이(크기) 제한
        - @Size(min = 4, max = 10) : 최소/최대값 설정 가능

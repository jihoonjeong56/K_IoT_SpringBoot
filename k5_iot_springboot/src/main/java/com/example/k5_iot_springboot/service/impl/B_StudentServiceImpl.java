...
public class B_StudentServiceImpl implements B_StudentService {
    private final B_StudentRepository studentRepository; // 생성자 주입 @RequiredArgsConstructor

    @Override
    @Transactional(readOnly = true) // 조회 계열(GET)은 Transactional 의 readOnly 옵션을 사용
    ...
    @Override
    @Transactional // 쓰기계열(POST, PUT, DELETE)는 기본 @Transactional 사용
    public StudentResponseDto createStudent(StudentCreateRequestDto requestDto) {
        ...
        return toDto(saved);
        // === 영속성 컨텍스트와 관리
        // : Transactional의 변경 감지(Dirty Checking)
        // - save 호출 없이도 트랜잭션 종료시 수정 가능

        //cf) UPDATE는 save 생략 가능 / CREATE는 save 생략 불가.
        // UPDATE 흐름
        // 1) findById()로 엔티티 조회: 영속성 컨텍스트 진입(1차 캐시되고, 관리 상태로 변경)
        // 2) 필드변경: 관리상태인 경우 JPA 는 엔티티의 스냅샷(조회 시점의 원본값)을 몰래 보관
        //              >> 비교후 Dirty Checking(변경감지)
        // 3) 데이터 변경 감시 시 : UPDATE SQL을 만들어 실행
        // 4) 자동으로 SQL 쿼리 실행

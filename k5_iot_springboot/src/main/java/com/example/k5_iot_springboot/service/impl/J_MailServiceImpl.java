...
public class J_MailServiceImpl implements J_MailService {
    ...
    private MimeMessage createEmail(String email, String token) throws MessagingException {
        ...
        message.setSubject("======이메일 인증링크발송(k5-springboot)");

        String body = """
                <h3>이메일 인증링크 입니다.</h3>
                <a href="http://localhost:8080/api/v1/auth/verify?token=%s">여기를 클릭하여 인증을 완료해 주세요</a>
                """.formatted(token);
    ...
    public void verifyEmail(String token) {
        ...
        System.out.println("이메일 인증이 완료되었습니다. -> "+ email);

...
/*
    ...
    2) Service(Business  계층)
        ...
        - 도메인 규칙(중복 검사, 상테정니, 권환 확인 등)을 처리
        - Repository 를 통해 DB에 접근
        +) DTO <<->> 엔티티 매핑도 발생
        +) createAt/updatedAt은 엔티티의 @PrePersist/@PreUpdate 대신 서비스 레벨에서 관리
        
    3) Repository(Persistence 계층)
        : JPA 를 통해 순수한 DB 접근만 담당
        - 쿼리 메서드, QueryDSL 등을 정의하고 비즈니스 로직은 절대 작성하면 안됨.

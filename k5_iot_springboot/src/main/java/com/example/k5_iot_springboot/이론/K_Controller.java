...
/*
    ...
    >> GET, PUT, DELETE에 사용(POST 사용하지 않는다)

    1) 리소스를 특정할 수 있는 PK 값을 주로 사용
    2) 경로 내에 중괄호{} 로 값을 감싸서 표현
    ...
    2. @RequestBody
    : 클라이언트의 HTTP 요청 본문(Body)에 담긴 JSON, XML 데이터를 자바 객체로 변환하여 메서드의 파라미터로 매핑할 때 사용
    - JSON, XML 형태의 데이터를 DTO 객체로 자동 변환 (RequestDto)

    @PostMapping
    Ex) public String createUser(@RequestBody UserCreateRequestDto dto){
        - 주로 POST, PUT, DELETE 요청에 주로 사용 (GET 사용 안함)
    }
    1) 반드시 요청 본문이 존해해야함(없으면 예외 발생)
    2) 클라이언트는 "Content-Type: application/json" 헤더 설정
    3) DTO 객체는 반드시 Getter/Setter 또는 @Data가 필요하다.

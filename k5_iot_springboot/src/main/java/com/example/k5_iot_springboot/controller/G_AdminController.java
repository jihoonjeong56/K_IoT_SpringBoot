...
@PreAuthorize("hasRole('ADMIN')") // 클래스 레벨의 권한
// @PreAuthorize("hasAnyRole('ADMIN', 'MANAGER')") - 복수 권한 체크
// +) controller 자체의 권한 부여도 가능하지만,
//      컨트롤러 내부의 각각의 메서드에 서로다른 권한 부여도 해당 어노테이션 사용
public class G_AdminController {
    ...
    // - 추가) 기존 권한 유지(기존 + 요청권한) - 권한을 확장/보강

    // 권한 갱신
    @PutMapping("/roles/replace") // 자원의 상태를 통째로 교체(덮어쓰기 - 동일한 요청을 여러번 보내도 결과가 같음)
    // : 멱등성

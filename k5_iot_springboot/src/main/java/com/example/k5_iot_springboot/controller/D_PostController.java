...
public class D_PostController {
    ...
    public ResponseEntity<ResponseDto<PostDetailResponseDto>> createPost(
            ...
            // - 사용자가 클라이언트로 부터 전달한 데이터가 미리 정의된 규칙에 맞는지 확인(검증)
    ) {
    ...
    @GetMapping(ApiMappingPattern.Posts.SEARCH_BY_COMMENT)
    ...
    @GetMapping("/author/{author}/min-commnets")
    public ResponseEntity<ResponseDto<List<PostWithCommentCountResponseDto>>> getAuthorPostsWithMinComments(
            @PathVariable("author") @NotBlank(message = "작성자는 비워질 수 없습니다.") String author,
            @RequestParam(name = "minCount", defaultValue = "1")
            @PositiveOrZero(message = "minCount는 0이상이어야 합니다.") int minCount
    ){

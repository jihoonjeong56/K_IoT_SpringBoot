...
/*
    ...
    - 다양한 어노테이션으로 CSRF 공격, 세년 고정 공격을 방어
    - 요청 헤더에 포함된 보안 처리도 가능

    1. 인증(Authentication)
        EX) 신분 확인
        : 사용자가 누구인지 확인하는 과정, 신원 입증 과정
        - 사용자가 로그인 기능을 통해 아이디, 비밀번호를 입력하면, 해당 데이터들을 기반으로 사용자가 주장하는 인물인지 확인


    2. 인가(Authorization)
        EX) 권한 확인
        : "인증된" 사용자가 특정 리소스(데이터)에 접근할 수 있는 권한이지 확인하는 과정
        - 관리자만 특정 페이지에 접근하거나 특정 데이터를 호출할 수 있도록 설정

    cf) CSRF(Cross-Site Request Forgery)
        : 자신(인증된 사용자)의 의지와는 상관업이 특정 웹 애플리케이션에 공격자가 원한느 요청을 보내도록 유도하는 웹 보안 취약점

    === Security 사용 목적 ===
    1. 보안기본기 자동 제공: 비밀번호 암호화, 세션/토큰 인증, 권한 체크, CSRF 보호 등
    2. 표준화된 구조: 보안 코드가 일관되고 유지보수 쉬움
    3. 유연한 확장성: 소셜 로그인, OAuth2, JWT, 2FA 등으로 확장 가능.
    4. 필수 방어막: 인증/인가 없는 서비스는 보안 취약

    === Security 선행 개념 ===
    1. HTTP 는 무상태(Stateless)
        : 인터넷의 HTTP 통신은 기억력이 없음
        - 서버는 이전 요청에 대한 사용자 정보를 기억하지 않음(매번 로그인 정보를 다시 제출해야함)
    2. 세션(Session) VS 토큰(JWT - Json Web Token)
        1) 세션: 서버에 "누가 로그인 했는지" 기록, 클라이언트 세션 ID 쿠키만 전달
            - 서버가 로그인 상태 기억
        2) JWT : 서버는 토큰만 검증하고 별도의 저장소가 없어도 됨(완전한 Stateless)
            - 서명된 토큰으로 인증
    3. 암호화 / 해싱
        - 비밀번호 암호화 (문자열 그대로 저장하면 보안성 저하)
            >> 해싱(Hashing): 긴 암호문 으로 변환
    ...
    3) AuthenticationManager/ Provider - 신분 확인 부서
        : 로그인 시도(/login)가 들어오면, 필터가 아이디/비번을 요청에서 꺼냄
            ...
            >> 여러 AuthenticationProvider 에게 일을 시킴( Manager가 Provider 에게)
                - 해당 아이디/비밀번호 일치 확인
                - DB 기반 로그인, 소셜 로그인, JWT 검증 등 다양한 Provider가 존재
    4) UserDetailsService - 사람찾기 창구
    ...
    5) PasswordEncoder - 비밀번호 검증 담당
        ...
        - 사용자가 입력한 비밀번호화 DB 해시값을 Encoder가 비교
            >> 일치하면 인증 성공, 틀리면 실패
    ...
    6) SecurityContext/SecurityContextHolder - 현재 로그인 정보 저장소
        : 인증 성공 시 스프링 시큐리티는 SecurityContext 안에 로그인된 유저 정보를 저장
            ...
            >> 서비스/컨트롤러 Authentication 타입의 매개변수로 로그인 사용자 정보 접근 가능

    7) Authorities (권한 처리) - 어디까지 접근할 수 있는지 확인
        : UserDetails 안에 유저가 가진 권한(ROLE) 정보 저장
        - 필터체인은 요청된 URL과 권한을 비교
            >> 맞으면 통과, 아니면 막힘

    8) EntryPoint / AccessDeniedHandler - 실패처리
        : 만약 인증이 안됐거나(로그인 실패), 권한이 부족

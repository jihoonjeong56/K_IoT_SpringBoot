...
/*
    ...
    5) 만료까지 남은 시간 계산 메서드 - getRemainingMillis 메서드


 */
@Component
// cf) @Component (클래스 레벨 선언) - 스프링 런타임 시 컴포넌트 스캔을 통해 자동으로 빈을 찾고 등록(의존성 주임)
...
public class JwtProvider {
    ...
    /**
     * 서명용 비밀키, 엑세스 토큰 만료시간(ms), 만료 직후 허용할 시계오차(s)
     */
    ...
    public JwtProvider(
        ...
        // 생성자 : JWTProvider 객체 생성시 비밀키와 만료시간 초기화
        // 키 강도 검증(Base64 디코딩 후 256 비트 이상 권장)
        byte[] secretBytes = Decoders.BASE64.decode(secret);
        if (secretBytes.length < 32) {
            // 32 byte = 256 bit
            // : HS256에 적정한 강도의 키를 강제하여 보안 강화
            throw new IllegalArgumentException("jwt.secret 은 항상 256비트 이상을 권장합니다.");
        }
        ...
        this.parser = Jwts.parser()
                .verifyWith(this.key) // 해당 키로 서명 검증을 수행하는 파서(이후 파싱마다 반복 설정 필요 없음)
                .build();
    }
    /* ========
     * 토큰 생성
     ...
     * @param roles    권한목록(중복 제거용 Set 권장) - JSON 배열로 직렬화
    ...
    public String generateJwtToken(String username, Set<String> roles) {
        ...
        return Jwts.builder()
                ...
                //.issuedAt(iat) // 표준 클레임에 현재 시간 설정(발행시간)
                .setIssuedAt(iat) // 표준 클레임에 현재 시간 설정(발행시간)
                //.expiration(exp) // 현재 시간에 만료시간을 던한 설정(만료시간)
                .setExpiration(exp) // 현재 시간에 만료시간을 던한 설정(만료시간)
    ...
    public String removeBearer(String bearerToken) {// 입력 : "Bearer <token>"
        if (bearerToken == null || !bearerToken.startsWith(BEARER_PREFIX)) {
            throw new IllegalArgumentException("Authorization 형식이 올바르지 앖습니다.");
        }
        // substring 인자가 한개인 경우: index 0 부터 인자값 "전"까지 잘라내기
    ...
    /**
     * 내부파싱(검증 포함) - 서명 검증 + 구조 검증한 뒤 Claims(페이로드)를 반환
     * >> 만료 시 clock-skew 허용 옵션
     */
    // 파싱: 텍스트나 데이터를 컴퓨터가 이해할 수 있는 특정 구조로 분석하고 변환하는 과정
    ...
    public boolean isValidToken(String tokenWithoutBearer) {
        // 검증 - 서명 불일치, 변소, 포맷 이상, 만료(허용 오차 초과) 등 모든 예외는 catch로 전달 - false 반환
        try {
            parseClaimsInternal(tokenWithoutBearer, true); // clock-skew 허용
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    public Claims getClaims(String tokenWithoutBearer) {
        // 유효성 검사 + 파싱을 한 번에 처리고, payload(Claims) 반환

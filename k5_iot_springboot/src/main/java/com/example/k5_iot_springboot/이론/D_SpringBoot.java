...
/*
    ...
    : 엔터프라이즈 애플리케이션 개발을 위한 자바 기반의 강력한 프레임 워크
    ...
    - 내장 WAS(Tomcat 등)를 포함: 빌드 설치 없이 실행 가능
    ...
    [클라이언트] - [서블릿 컨테이너] - [Dispatcher Servlet]
                                        |
                        (뷰 / 핸들러 매핑 / 핸들러 어댑터 / 뷰 리볼버)
                                            |
                                          컨트롤러

    1) 클라이언트 >> 서블릿 컨테이너 >> Dispatcher Servlet(DS)
        : 클라이언트에서 "요청"이 들어오면, 서블릿 컨테이너(Tomcat)가 URL 매핑에 따라 해당 요청을 DS로 전달

    2) DS >> 핸들러 매핑
        : 해당 요청(URL, HTTP 메서드 등) 에 대응하는 핸들러(= 컨트롤러메서드)를 찾음
        - 핸들러 매핑은 적합한 컨트롤러/핸들러 정보를 찾아서 반환

    3) DS >> 핸들러 어댑터
        : DS가 선택된 핸들러를 실행할 수 있는 어댑터를 찾아 핸들러 어댑터를 호출
        - 해당 어댑터는 실제 컨드롤러와 연결되어 컨트롤러의 메서드를 실행하고, 실행 결과(모델 & 뷰), DS에 반환
           >> 어댑터
    ...
    5) DS >> 뷰 리볼버
        컨트롤러 결과가 뷰 이름이면, DS는 뷰 리볼버에게 실제 뷰 객체로 바꿀 것을 요청 (JSP/Thymeleaf 등)

    6) DS >> 뷰
        : DS 가 뷰 객체에 모델 데이터를 넘겨 렌더링을 수행(HTML 등이 서블릿 컨테이너를 통해 클라이언트의 응답으로 전송)


    4. DispatcherServlet(디스패쳐 서블릿) - dispatch : 보내다
    : 스프링부트의 핵심 서블릿
    - 클라이언트의 요청을 받아 "알맞은 핸들러(controller)"를 찾고 실행하며, 처리된 결과를 다시 클라이언트에게 반환
...
/*
    ...
    1. 서블릿 (Servlet)
        ...
        2) 기능/URI 마다 여러 서블릿을 만들거나, 직접 프론트 컨트롤러 패턴(서블릿1개 + 내부 분기)을 구현
    2. 스프링부트(MVC)
